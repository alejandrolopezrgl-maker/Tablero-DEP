...
# INICIALIZACIÓN DE VARIABLES DE SESIÓN PILAR POR PILAR PARA CONTROL TOTAL DE SIMULACIÓN
...
if "sim_pilar_especiales" not in st.session_state: st.session_state.sim_pilar_especiales = None
if "sim_pilar_usados" not in st.session_state: st.session_state.sim_pilar_usados = None
if "sim_pilar_esg" not in st.session_state: st.session_state.sim_pilar_esg = None
...
# 4. FÓRMULA DE PROYECCIÓN DEL SCORE GLOBAL CON LAS PONDERACIONES EXACTAS DEL MANUAL TASA
...
# DATAFRAME OPERATIVO COMPLETO ACOPLADO AL 100% CON LAS VARIABLES SIMULADAS
data_operativa = {
...
# MOTOR DE RANKING ELÁSTICO RED COMPLETO DESBLOQUEADO HASTA EL PUESTO 1
if score_global_final <= 62.0:
    puesto_calculado = 24
elif score_global_final >= 99.9:
    puesto_calculado = 1
elif score_global_final >= 72.3:
    puesto_calculado = int(5 - ((score_global_final - 72.3) / (100.0 - 72.3)) * (5 - 1))
    puesto_calculado = max(1, min(5, puesto_calculado))
else:
...
if st.sidebar.button("🔄 Restablecer Valores Oficiales", key="btn_reset_lateral"):
    ...
    st.session_state.sim_pilar_especiales = None
    st.session_state.sim_pilar_usados = None
    st.session_state.sim_pilar_esg = None
...
with tab_calidad:
    ...
    df_p["Jujuy_Menciones"] = [42, 28, 19, 14, 9, 7, 5, 3, 2, 0]
    df_p["Salta_Menciones"] = [25, 45, 30, 12, 15, 8, 4, 6, 1, 1]
    df_p["Tartagal_Menciones"] = [18, 10, 8, 14, 5, 6, 2, 1, 3, 0]
...
with tab_plan:
    ...
    with c_btn2:
        if st.button("🧹 Limpiar Simulación"):
            st.session_state.sim_pilar_ventas = None; st.session_state.sim_pilar_posventa = None; st.session_state.sim_pilar_tpa = None
            st.session_state.sim_pilar_kinto = None; st.session_state.sim_pilar_tcfa = None; st.session_state.sim_pilar_general = None
            st.session_state.sim_pilar_especiales = None; st.session_state.sim_pilar_usados = None; st.session_state.sim_pilar_esg = None
            if "db_plan_puro_19_v2" in st.session_state: del st.session_state.db_plan_puro_19_v2
