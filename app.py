...
# INICIALIZACIÓN DE VARIABLES DE SESIÓN PILAR POR PILAR
...
# AUTOMATIZACIÓN DE COMPONENTES EMT DESDE EL backend
...
# ¡SINCRO AUTOMATIZADA COMPLETADA!: Fuerza el retorno de los controles EMT al 100% de cumplimiento de forma cruzada
...
with tab_dashboard:
    ...
    # ¡SANEADO!: Los sliders ahora leen de forma obligatoria las variables del session_state para automatizar el reset masivo
    with col_em1:
        st.session_state.emt_a = st.slider("A: Estructura Central (100)", 0, 100, st.session_state.emt_a, key="slider_a_v2")
        st.session_state.emt_b = st.slider("B: Servicio al Cliente (100)", 0, 100, st.session_state.emt_b, key="slider_b_v2")
        st.session_state.emt_c = st.slider("C: Kinto Movilidad (100)", 0, 100, st.session_state.emt_c, key="slider_c_v2")
    with col_em2:
        st.session_state.emt_d = st.slider("D: Club Toyota (100)", 0, 100, st.session_state.emt_d, key="slider_d_v2")
        st.session_state.emt_e = st.slider("E: Toyota Plan de Ahorro (100)", 0, 100, st.session_state.emt_e, key="slider_e_v2")
        st.session_state.emt_f = st.slider("F: Toyota Financial Services (100)", 0, 100, st.session_state.emt_f, key="slider_f_v2")
    with col_em3:
        st.session_state.emt_g = st.slider("G: Vehículos Usados (100)", 0, 100, st.session_state.emt_g, key="slider_g_v2")
        st.session_state.emt_h = st.slider("H: Canal Convencional (100)", 0, 100, st.session_state.emt_h, key="slider_h_v2")
        st.session_state.emt_i = st.slider("I: Services Conectados (100)", 0, 100, st.session_state.emt_i, key="slider_i_v2")
    
    if porcentaje_emt < 80.0:
        st.error(f"🚨 Alerta DEP: Estándar EMT Asegurado en {total_puntos_emt} / 900 puntos ({porcentaje_emt:.1f}%). Por debajo del umbral mínimo del 80%. Penalidad activa.")
    else:
        st.success(f"🎉 Estándar EMT Certificado: {total_puntos_emt} / 900 puntos ({porcentaje_emt:.1f}%). Concesionario a salvo.")
...
with tab_calidad:
    ...
    df_p["Salta_Menciones"] = [15, 38, 22, 5, 12, 9, 4, 1, 3, 2]
    df_p["Tartagal_Menciones"] = [29, 11, 8, 18, 5, 4, 6, 2, 0, 1]
    ...
    fig_pareto.update_layout(title=f"Diagrama de Pareto de Calidad - Sucursal {sucursal}", xaxis=dict(title="Categorías Críticas", tickangle=-25), yaxis=dict(title="Número de Quejas (Cantidad)"), yaxis2=dict(title="Porcentaje Acumulado %", overlaying="y", side="right", range=[0, 100]), legend=dict(orientation="h", yanchor="top", y=-0.45, xanchor="center", x=0.5), margin=dict(b=140), height=550)
...
with tab_plan:
    ...
    if "db_plan_puro_19_v2" not in st.session_state:
    ...
    df_ed_2 = st.data_editor(st.session_state.db_ampliacion_2_limpia, use_container_width=True, key="grilla2_clean", hide_index=True, column_config={"#": st.column_config.NumberColumn(disabled=True), "Código Auditoría Manual": st.column_config.TextColumn(disabled=True), "Gerencia / Sector": st.column_config.TextColumn(disabled=True), "Tema / Proyecto": st.column_config.TextColumn(disabled=True), "Situación actual": st.column_config.TextColumn(disabled=True), "Acción Correctiva": st.column_config.TextColumn(disabled=True), "Prioridad": st.column_config.TextColumn(disabled=True), "Indicador / Entregable": st.column_config.TextColumn(disabled=True), "Responsable": st.column_config.TextColumn(disabled=True), "Estado": st.column_config.SelectboxColumn(options=["PENDIENTE", "EN PROCESO", "COMPLETADO"]), "Objetivo Simulación (%)": st.column_config.NumberColumn(min_value=0.0, max_value=100.0, format="%.1f%%")})

    c_btn1, c_btn2 = st.columns(2)
    with c_btn1:
        if st.button("🧮 Simular e Impactar Dashboard"):
            st.session_state.db_dep_final_oficial_2026_v10 = df_ed_1; st.session_state.db_ampliacion_2_limpia = df_ed_2
    ...
    with c_btn2:
        if st.button("🧹 Limpiar Simulación"):
            # ¡SANEADO TOTAL MASIVO!: Al limpiar el plan de acción, también obliga a los deslizadores EMT a saltar al 100%
            for k in ["sim_pilar_ventas", "sim_pilar_posventa", "sim_pilar_tpa", "sim_pilar_kinto", "sim_pilar_tcfa", "sim_pilar_general", "sim_pilar_especiales", "sim_pilar_usados", "sim_pilar_esg"]: st.session_state[k] = None
            for ek in ["emt_a", "emt_b", "emt_c", "emt_d", "emt_e", "emt_f", "emt_g", "emt_h", "emt_i"]: st.session_state[ek] = 100
            if "db_dep_final_oficial_2026_v10" in st.session_state: del st.session_state.db_dep_final_oficial_2026_v10
            if "db_ampliacion_2_limpia" in st.session_state: del st.session_state.db_ampliacion_2_limpia
            st.success("🧹 Reset completado."); st.rerun()

    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        df_ed_1.to_excel(writer, sheet_name='Plan de Accion Oficial', index=False)
        df_ed_2.to_excel(writer, sheet_name='Ampliacion DEP', index=False)
