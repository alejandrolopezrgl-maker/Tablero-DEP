...
if st.sidebar.button("🔄 Restablecer Valores Oficiales", key="btn_reset_lateral"):
    for k in ["sim_pilar_ventas", "sim_pilar_posventa", "sim_pilar_tpa", "sim_pilar_kinto", "sim_pilar_tcfa", "sim_pilar_general", "sim_pilar_especiales", "sim_pilar_usados", "sim_pilar_esg", "db_plan_puro_v20", "db_ampliacion_v20"]: 
        if k in st.session_state: st.session_state[k] = None
    st.rerun()

# --- AQUÍ SE DECLARAN LAS 6 PESTAÑAS ---
...
# =======================================================
# 2. PESTAÑA: EVOLUCIÓN JUNIO VS JULIO (NUEVA)
# =======================================================
with tab_evolucion:
    st.subheader("📈 Comparativo Evolutivo Oficial: Junio 2026 vs. Julio 2026")
    st.markdown("Diagnóstico de brechas y avances según los informes oficiales emitidos por Toyota Argentina:")
    ...
    fig_comp.update_layout(title="Comparativa de Cumplimiento por Unidad de Negocio (Junio vs Julio 2026)", barmode="group", yaxis=dict(range=[0, 110]), legend=dict(orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5))
...
with tab_plan:
    ...
    if "db_plan_puro_v20" not in st.session_state or st.session_state.db_plan_puro_v20 is None:
        ...
        st.session_state.db_plan_puro_v20 = pd.DataFrame(rows, columns=["#", "Código Auditoría Manual", "Gerencia / Sector", "Tema / Proyecto", "Situación actual", "Acción Correctiva", "Prioridad", "Indicador / Entregable", "Responsable", "Estimación de Cumplimiento", "Fecha Estimada Cumplimiento", "Estado", "% Avance Acción (Simulación)"])

    df_ed_1 = st.data_editor(st.session_state.db_plan_puro_v20, use_container_width=True, key="grilla1_v20", hide_index=True)

    c_btn1, c_btn2 = st.columns(2)
    with c_btn1:
        if st.button("🧮 Simular e Impactar Dashboard"):
            st.session_state.db_plan_puro_v20 = df_ed_1
    ...
    with c_btn2:
        if st.button("🧹 Limpiar Simulación"):
            for k in ["sim_pilar_ventas", "sim_pilar_posventa", "sim_pilar_tpa", "sim_pilar_kinto", "sim_pilar_tcfa", "sim_pilar_general", "sim_pilar_especiales", "sim_pilar_usados", "sim_pilar_esg", "db_plan_puro_v20"]: 
                if k in st.session_state: st.session_state[k] = None
...
with tab_docs:
    ...
    st.info("Documentos oficiales de auditoría y evaluación del Manual DEP 2026.")
